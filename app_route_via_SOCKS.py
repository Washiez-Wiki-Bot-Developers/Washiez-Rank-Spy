...
async def fetch_users_in_role(session, group_id, role_id, role_member_count="?"):
    ...
    # users, cursor = await pickup_unfinished_data(role_id)
    ...
    while True:
        ...
        try:
            async with session.get(
                ...
                if response.status != 200:
                    logger.error(
                        f"\nFailed to fetch users for role {role_id}: {response.status}"
                    )
                    return users
                data = await response.json()
                ...
                if (
                    not (role_member_count is None or role_member_count == "?")
                    and role_member_count != 0
                ):
                    try:
                        percentage: Union[str, float] = (
                            math.ceil((len(users) / int(role_member_count) * 100) * 100)
                            / 100
                        )  # pyright: ignore[reportRedeclaration]
                    except ZeroDivisionError as e:
                        logger.error(
                            f"ZeroDivisionError while calculating percentage for role {role_id}: {e}"
                        )
                        percentage: Union[str, float] = "?"
                else:
                    percentage = "?"

                if len(users) == role_member_count:
                    logger.debug(
                        f"Fetched all {role_member_count} users for role {role_id}."
                    )
                    percentage = 100.0
                ...
                sys.stdout.write(
                    f"\rFetched {len(data['data'])} users for role {role_id}. Currently have {len(users)}/{role_member_count} ({percentage}%) users. | Time Elapsed: {time.strftime('%H:%M:%S', time.gmtime(elapsed_time))} | Zulu time: {time.strftime('%H:%M:%S', time.gmtime(time.time()))}"
                )
                sys.stdout.flush()
            ...
            break
    sys.stdout.write("\r\xe2\x80\x8b")
    sys.stdout.flush()
    logger.info(f"Completed fetching. We have {len(users)} users for role {role_id}.")
...
async def fling_test(interaction: discord.Interaction):
    ...
    await interaction.edit_original_response(
        content=f"🧼 Foam response time: {latency} ms\nRequest received at {received_rough:.3f} sec — suds flow optimal 🫧"
    )
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Game("Monitoring role changes.Where are my socks??"))
